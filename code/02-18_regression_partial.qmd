...
but it could aslo be done as through ANOVA

for ANOVA you need y varaibel sthats numerical or categorical and x thats categorical

for regression you need a nuerical y



If we were interested on mean yield based on seeding rates ANOVA would be good to use
then you could look at seding rates effects on mean effects on yield 


regression is usuful for looking at a potential rate that may be between two of our treatment/ experiments seeding rates
...
we are using pearson isntead of r_stud

the y axis on residual lt would be on the same scale as __ plot so we are scaling 
...
but this alone doesnt teel us if its a good or bad model for our data
...
```
we would want our residueals one the line, but low seedng rates are bleow and high are above the line , so the model isnt properly specified 
...
ggplot(reg_dfw, 
  ...
  geom_line(data = nd, 
            ...
            color = "forestgreen") +
  geom_ribbon(data = nd, aes(ymin = mod1_int_lcl,
                             ymax = mod1_int_upl),
               x = sr_ksha, 
              inherit.aes = FALSE,
              alpha = .5
...
# Model fitting
mod2_lin <- lmer(yield_mgha ~ (1|rep) + sr_ksha, data = reg_dfw)
